...
/** Implements a Java Client for the Client/Server demo. For details about the protocol between client and server,
 * consult the file <code>Protocol.txt</code>. */
public class Client {
	...
	/** Constructs a default Client. It connects to the speech server, and constructs an AudioPlayer. */
	public Client() {
		if (!connect()) {
	 ...
	 * @return <code>true</code> if successfully connected <code>false</code> if failed to connect */
	...
	/** Sends the given line of text to the Socket, appending an end of line character to the end.
	 ...
	 * @return <code>true</code> if the TTS transaction was successful <code>false</code> if an error occurred */
	private boolean sendTTSRequest(String text) {
		...
		if (metrics) {
			System.out.println("FirstByte : " + (receiveTime - sendTime) + " ms");
		}

		return true;
	}

	/** Reads the given number of bytes from the socket, and plays them with the AudioPlayer.
