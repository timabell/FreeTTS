...
/** Implements a text-to-speech server for the Client/Server demo. It creates two Voices when it starts up, one 8k and
 * one 16k, and then waits for socket connections. After it receives a connection, it waits for TTS requests from the
 * client, does speech synthesis, and then sends the synthesized wave bytes back to the client. For a complete
 * specification of the protocol, please refer to the document <code>Protocol.txt</code>. */
public class Server extends TTSServer {
	...
	/** Constructs a default Server, which loads an 8k Voice and a 16k Voice by default. */
	public Server() {
...
class SocketTTSHandler implements Runnable {
	...
	/** Constructs a SocketTTSHandler with the given <code>Socket</code> and <code>Server</code>.
