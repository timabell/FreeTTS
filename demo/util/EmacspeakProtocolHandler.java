...
/** Implements a very simplified (and incomplete) version of the Emacspeak speech server. See the Emacspeak protocol
 * document at http://emacspeak.sourceforge.net/info/html/TTS-Servers.html for more information. */
public abstract class EmacspeakProtocolHandler implements Runnable {
	...
	/** Sometimes emacspeak will embed DECTalk escape sequences in the text. These sequences are not meant to be spoken,
	 * and FreeTTS currently does not interpret them. This simple flag provides a mechanism for FreeTTS to cut strings of
	 * the form "[...]" out of text to be spoken (DECTalk escape sequences are of the form "[...]"). Since this is a
	 * relatively heavy-handed thing to do, this feature is turned off by default. To turn it on, add -DstripDECTalk=true
	 * to the command line. */
	...
	/** Returns true if the given input string starts with the given starting and ending sequence.
	 * @param start the starting character sequence
	 * @param end the ending character sequence
	 * @return true if the input string matches the given Pattern; false otherwise */
	...
	/** Returns the text of the given input that is within curly brackets. If there are no curly brackets (allowed in the
	 * Emacspeak protocol if the text has no spaces), then it just returns the text after the first space.
	...
	/** Strips DECTalk commands from the input text. The DECTalk commands are anything inside "[" and "]". */
			...
			while (isSocketLive()) {
				...
				if (command != null) {
					...
					if (commandType == EXIT_COMMAND) {
						...
						try {
							setRate(Float.parseFloat(textInCurlyBrackets(command)));
	...
	private boolean isSocketLive() {
		return (socket.isBound() && !socket.isClosed() && socket.isConnected() && !socket.isInputShutdown() && !socket
				.isOutputShutdown());
	}
	...
	/** Detects and handles a possible emacspeak quitting sequence of commands, by looking at the given command type and
	 * content. If a quitting sequence is detected, it will close the socket. Note that this is not the best way to trap a
	 * quitting sequence, but I can't find another way to trap it. This method will do a socket.notifyAll() to tell
	 * objects waiting on the socket that it has been closed.
	...
	private synchronized void detectQuitting(int commandType, String content) throws IOException {
		if (commandType == QUEUE_COMMAND) {
			...
			if (content.equals("no")) {
				lastQueuedCommand = "";
			} else if (content.equals("yes") && lastQueuedCommand.startsWith(stopQuestionStart)) {
				socket.close();
				notifyAll();
			}
		}
	...
	/** Prints the given message if the <code>debug</code> System property is set to <code>true</code>.
