...
public class UnitDatabase {
	...
	void dumpUnitCatalog(PrintStream out) {
		...
		while (keys.hasNext()) {
			...
			out.println("UNIT_TYPE " + key + " " + currentIndex + " " + units.size());
	...
	void dumpVoiceData(PrintStream stsOut, PrintStream mcepOut) throws IOException {
		int sampleRate = 0;
		...
		stsOut.println("STS STS " + numFrames + " " + numLPCChannels + " " + sampleRate + " " + lpcMin + " " + lpcRange
				+ " 0.000000 1"); /* postEmph and residualFold */

		mcepOut.println("STS MCEP " + numFrames + " " + numMCEPChannels + " " + sampleRate + " " + mcepMin + " "
				+ mcepRange + " 0.000000 1"); /* postEmph and residualFold */
	...
	public void dumpUnitIndex(PrintStream unitIndexOut, PrintStream stsOut, PrintStream mcepOut) throws IOException {
		...
		while (keys.hasNext()) {
			...
			for (int i = 0; i < units.size(); i++) {
				...
				unitIndexOut.println("UNITS " + unitTypeIndex + " " + phoneNumber + " " + (startIndex + track.startIndex) + " "
						+ (endIndex + track.startIndex) + " " + ((unit.previous != null) ? unit.previous.index : 65535) + " "
	...
	/** Testing. args[0] = *.catalog file args[1..n] = * files (no path or suffix - the code will add sts/file.sts and
	 * mcep/file.mcep.txt */
	static public void main(String[] args) {
		try {
			...
			/*
			 * Store the TrackFile in the sts and mcep HashMaps indexed by the filename. */
			...
			database.dumpUnitIndex("FreeTTS/unit_index.txt", "FreeTTS/sts.txt", "FreeTTS/mcep.txt");
