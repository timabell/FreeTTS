...
public class Track {
	...
	/*
	 * Set up later - says where this track is in the big monolithic sts and mcep files. */
	...
	public Track(String filename, int type, float min, float range) throws IOException {
		this.min = min;
		this.range = range;
		if (type == STS) {
			readSTS(filename);
	...
	void readMCEP(String filename) throws IOException {
		...
		while (!line.equals("EST_Header_End")) {
			...
			if (line.startsWith("NumFrames")) {
				...
				/*
				 * With MCEP, the first channel is the energy. We drop this because it is not used to select units. */
	...
	public int findTrackFrameIndex(float time) {
		...
		while ((frameNum < (frames.length - 1))
				&& (Math.abs(time - frames[frameNum].pitchmarkTime) > Math.abs(time - frames[frameNum + 1].pitchmarkTime))) {
			frameNum++;
		}
		return frameNum;
	}
	...
	/** For testing. args[0] = filename args[1] = type (1 = STS, 2 = MCEP) */
