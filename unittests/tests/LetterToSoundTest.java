...
public class LetterToSoundTest extends TestCase {
	...
	protected void setUp() {
		try {
			lts = new LetterToSoundImpl(new URL("file:bld/classes/com/sun/speech/freetts/en/us/cmulex_lts.bin"), true);
	...
	public void testLTS() {
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				assertTrue("Phones returned for " + word + " is not null: ", lts_phone_array != null);
				...
				lts_phones.append(")");
				assertTrue("Phones returned for " + word + " are identical " + "(Our phones: " + lts_phones + ", "
						+ "Flite phones: " + flite_phones + "): ", flite_phones.equals(lts_phones.toString()));
			}
		} catch (IOException e) {
	...
	/* Tests to see if the binary version of the database matches that of the text database. public void testBinaryLoad()
	 * { try { LetterToSoundImpl text = new LetterToSoundImpl( new URL("file:../en/us/cmulex_lts.txt"), false);
	 * LetterToSoundImpl binary = new LetterToSoundImpl( new URL("file:../en/us/cmulex_lts.bin"), true);
	 * assertTrue("text binary compare", text.compare(binary)); } catch (IOException ioe) { fail("Can't load lts " + ioe);
