...
public class UtteranceTests extends TestCase {
	...
	protected void setUp() {
		...
		StringTokenizer tok = new StringTokenizer("January 1st 2001 How now brown cow "
		...
		for (Item item = tokens.getHead(); item != null; item = item.getNext()) {
			if (item.getFeatures().getString("name").equals("2001")) {
				...
				word.getFeatures().setString("name", item.getFeatures().getString("name").toLowerCase());
	...
	private void wordSylSeg(Utterance u) {
		...
		for (Item word = u.getRelation("Words").getHead(); word != null; word = word.getNext()) {
	...
	public void testFeature1() {
		...
		assertTrue("name", item.findFeature("name").toString().equals("u"));
		assertTrue("n.name", item.findFeature("n.name").toString().equals("a"));
		assertTrue("p.name", item.findFeature("p.name").toString().equals("n"));
		assertTrue("R:SylStructure.parent", item.findItem("R:SylStructure").toString().equals("u"));
		assertTrue("R:SylStructure.parent", item.findItem("R:SylStructure.parent").toString().equals("january"));
		assertTrue("R:SylStructure.parent.name", item.findFeature("R:SylStructure.parent.name").toString()
				.equals("january"));
		assertTrue("R:SylStructure.parent.n", item.findItem("R:SylStructure.parent.n").toString().equals("1st"));

		Item token = utterance.getRelation("tokens").getHead();
		assertTrue("token head", token.toString().equals("January"));
		assertTrue("n.n.name", token.findFeature("n.n.name").toString().equals("2001"));
		assertTrue("n.n.daughter.name", token.findFeature("n.n.daughter.name").toString().equals("two"));
		assertTrue("n.n.daughter.n.name", token.findFeature("n.n.daughter.n.name").toString().equals("thousand"));
		assertTrue("n.n.daughtern.name", token.findFeature("n.n.daughtern.name").toString().equals("one"));
	}
	...
	public void testUtteranceRelations() {
		assertTrue("Utterance Created", utterance != null);
		Relation tokens = utterance.createRelation("tokens");
		assertTrue("retrieved token relation missing", utterance.getRelation("tokens") == tokens);
		assertTrue("token relation missing", utterance.hasRelation("tokens"));
		assertTrue("missing relation found", !utterance.hasRelation("missing_relation"));
		assertTrue("retrieved missing token relation found", utterance.getRelation("missing_relation") == null);
	}
	...
	public void testUtteranceFeatures() {
		...
		assertTrue("present feature retreived", utterance.getString("is_present").equals("here I am"));
		assertTrue("missing voice feature found", !utterance.isPresent("voice_feature"));
		voice.getFeatures().setString("voice_feature", "is_set");
		assertTrue("voice feature missing", voice.getFeatures().isPresent("voice_feature"));
		assertTrue("voice feature missing", utterance.isPresent("voice_feature"));
		assertTrue("voice present feature retreived", utterance.getString("voice_feature").equals("is_set"));
	...
	public void testRelations() {
		...
		index = 0;
		for (Item item = r.getHead(); item != null; item = item.getNext(), index++) {
			assertTrue("Proper items", items[index] == item);
		}

		assertTrue("Items not equal", !items[0].getSharedContents().equals(items[1].getSharedContents()));
		Item dup = r.appendItem();
		Item dup2 = r.appendItem(dup);
		assertTrue("Items should be equal", dup.getSharedContents().equals(dup2.getSharedContents()));
	}
	...
	public void testItems() {
		...
		assertTrue("dup equals", dup2.getSharedContents().equals(i2.getSharedContents()));
		...
		assertTrue("r1,r2 equal", r1i1.getSharedContents().equals(r2i1.getSharedContents()));
		...
		assertTrue("Missing feature found", !r1i1.getFeatures().isPresent("not_present"));
		r1i1.getFeatures().setString("is_present", "here I am");
		assertTrue("presnt feature found", r1i1.getFeatures().isPresent("is_present"));
		assertTrue("present feature retreived", r1i1.getFeatures().getString("is_present").equals("here I am"));
		assertTrue("missing voice feature found", !r1i1.getFeatures().isPresent("voice_feature"));

		r1i1.getFeatures().setFloat("pi", (float) Math.PI);
		assertTrue("float get", r1i1.getFeatures().getFloat("pi") == (float) Math.PI);
		assertTrue("float get r2", r2i1.getFeatures().getFloat("pi") == (float) Math.PI);
