...
/** A simple dummy voice as a starting point for non-US-English cluster unit voices. All NLP stuff would need to be
 * implemented in order for this to become a full TTS voice. */
public class DiphoneVoice extends Voice implements ConcatenativeVoice {
	...
	public DiphoneVoice(String name, Gender gender, Age age, String description, Locale locale, String domain,
			String organization, Lexicon lexicon, URL database) {
		this(name, gender, age, description, locale, domain, organization, lexicon, database, null, null);
	}

	/** Creates a ClusterUnitVoice
	 * @param database the database of the voice
	 * @param unitNamer specifies the name of the Units (if null, an ldom naming scheme will be used: 'ae_afternoon')
	 * @param phonesetURL leads to the phoneset, which will be used for the FeatureProcessors (can be null)
	 * @param partOfSpeechURL leads to the pos-textfile which will be used for the FeatureProcessors (can be null) */
	public DiphoneVoice(String name, Gender gender, Age age, String description, Locale locale, String domain,
			String organization, Lexicon lexicon, URL database, URL phonesetURL, URL partOfSpeechURL) {
	 ...
	 * behaviors. There is no default unit selector
	 * @return the post lexical processor
	 * @throws IOException if an IO error occurs while getting processor */
