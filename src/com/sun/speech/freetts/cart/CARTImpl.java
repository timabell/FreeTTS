...
/** Implementation of a Classification and Regression Tree (CART) that is used more like a binary decision tree, with
 * each node containing a decision or a final value. The decision nodes in the CART trees operate on an Item and have
 ...
 * </pre>
 * <p>
 * Where <code>feat</code> is an string that represents a feature to pass to the <code>findFeature</code> method of an
 * item.
 * <p>
 * The <code>value</code> represents the value to be compared against the feature obtained from the item via the
 * <code>feat</code> string. The <code>operand</code> is the operation to do the comparison. The available operands are
 * as follows:
 * <ul>
 ...
 * <li>IN - [[[TODO: still guessing because none of the CART's in Flite seem to use IN]]] the value is in the list
 * defined by the feature.
 * </ul>
 * <p>
 * [[[TODO: provide support for the IN operator.]]]
 * <p>
 * For &lt; and >, this CART coerces the value and feature to float's. For =, this CART coerces the value and feature to
 * string and checks for string equality. For MATCHES, this CART uses the value as a regular expression and compares the
 * obtained feature to that.
 * <p>
 * A CART is represented by an array in this implementation. The <code>qfalse</code> value represents the index of the
 * array to go to if the comparison does not match. In this implementation, qtrue index is always implied, and
 * represents the next element in the array. The root node of the CART is the first element in the array.
 * <p>
 * The interpretations always start at the root node of the CART and continue until a final node is found. The final
 * nodes have the following form:
 * 
 * <pre>
 *   LEAF value
 * </pre>
 * <p>
 * Where <code>value</code> represents the value of the node. Reaching a final node indicates the interpretation is over
 * and the value of the node is the interpretation result. */
public class CARTImpl implements CART {
	/** Logger instance. */
	private static final Logger LOGGER = Logger.getLogger(CARTImpl.class.getName());
	/** Entry in file represents the total number of nodes in the file. This should be at the top of the file. The format
	 * should be "TOTAL n" where n is an integer value. */
	final static String TOTAL = "TOTAL";

	/** Entry in file represents a node. The format should be "NODE feat op val f" where 'feat' represents a feature, op
	 * represents an operand, val is the value, and f is the index of the node to go to is there isn't a match. */
	final static String NODE = "NODE";

	/** Entry in file represents a final node. The format should be "LEAF val" where val represents the value. */
	final static String LEAF = "LEAF";

	/** OPERAND_MATCHES */
	final static String OPERAND_MATCHES = "MATCHES";

	/** The CART. Entries can be DecisionNode or LeafNode. An ArrayList could be used here -- I chose not to because I
	 ...
	 * href="http://www.graphviz.org/">http://www.graphviz.org/</a>. If installed, call it as "dot -O -Tpdf *.dot" from
	 * the console to generate pdfs.
	 * </p>
	...
	public void dumpDot(PrintWriter out) {
		...
		for (Node n : cart) {
			out.println("\tnode" + Math.abs(n.hashCode()) + " [ label=\"" + n.toString() + "\", color=" + dumpDotNodeColor(n)
	 ...
	 * @return the CART
	 * @throws IOException if an error occurs during output Note that cart nodes are really saved as strings that have to
	 * be parsed. */
	...
	/** Creates a node from the given input line and add it to the CART. It expects the TOTAL line to come before any of
	 * the nodes.
	...
	abstract static class DecisionNode extends Node {
		...
		/** Returns the next node based upon the descision determined at this node
		...
		/** Get the next Node to go to in the CART. The return value is an index in the CART. */
	...
	static class ComparisonNode extends DecisionNode {
		...
		/** Compare the given value and return the appropriate Node index. IMPLEMENTATION NOTE: LESS_THAN and GREATER_THAN,
		 * the Node's value and the value passed in are converted to floating point values. For EQUAL, the Node's value and
		 * the value passed in are treated as String compares. This is the way of Flite, so be it Flite.
		...
		private String trace(Object value, boolean match, int next) {
			return "NODE " + getFeature() + " [" + value + "] " + comparisonType + " [" + getValue() + "] "
					+ (match ? "Yes" : "No") + " next " + next;
		}

		/** Get a string representation of this Node. */
		public String toString() {
			return "NODE " + getFeature() + " " + comparisonType + " " + getValueString() + " " + Integer.toString(qtrue)
	...
	static class MatchingNode extends DecisionNode {
		...
		/** Get a string representation of this Node. */
		public String toString() {
			StringBuffer buf = new StringBuffer(NODE + " " + getFeature() + " " + OPERAND_MATCHES);
