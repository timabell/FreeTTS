...
/** Determines duration timing for <code>Relation.SEGMENT</code> relations in an utterance. Annotates the
 * <code>Relation.SEGMENT</code> relation with an "end" time feature.
 * <p>
 * [[[TODO: The mean words-per-minute rate should become part of the CART data. For now, it is passed into the
 * constructor.]]]
...
public class Durator implements UtteranceProcessor {
	...
	private static final PathExtractor DURATION_STRETCH_PATH = new PathExtractorImpl(
			"R:SylStructure.parent.parent.R:Token.parent.local_duration_stretch", true);
	...
	/** Annotates the <code>Relation.SEGMENT</code> relations with cumulative "end" time features based on phone durations.
	 * Expects the CART to return a zscore for each phone, which specifies the number of standard deviations from the
	 * mean. This is coupled with a phone durations table that returns the mean and standard deviation for phones.
	...
	public void processUtterance(Utterance utterance) throws ProcessException {
		...
		//
		for (Item segment = utterance.getRelation(Relation.SEGMENT).getHead(); segment != null; segment = segment.getNext()) {
