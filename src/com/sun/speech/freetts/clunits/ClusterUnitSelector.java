...
public class ClusterUnitSelector implements UtteranceProcessor {
	 ...
	 * database, otherwise, its assumed that its a text database1
	 ...
	 * @param url the URL for the unit database. If the URL path ends with a '.bin' it is assumed that the DB is a binary
	 * database, otherwise, its assumed that its a text database1
	 * @param unitNamer an optional unit namer, specifying how the cluster units are called in the voice database
	...
	/** Generates the Unit Relation from the Segment Relation. <br>
	 * <b>Implementation note:</b><br>
	 * Populates the segment relation with segment names of the form: XX_YY where XX is the segment name (typically a
	 * phoneme) and YY is the word that the segment is in (stripped and lower case). The first step in cluster unit
	 * selection is to determine the unit type for each unit in the utterance. The unit type for selection in the simple
	 * talking clock example (cmu_time_awb) is done per phone. The unit type consists of the phone name followed by the
	 * word the phone comes from (e.g., n_now for the phone 'n' in the word 'now'). Invoke the Viterbi algorithm (via a
	 * viterbi class) that selects the proper units for the segment and adds that to each segment item. For each segment,
	 * create a unit and attach features based upon the selected units.
	 * @param utterance the utterance to generate the Unit Relation
	 * @throws ProcessException if an IOException is thrown during the processing of the utterance */
	public void processUtterance(Utterance utterance) throws ProcessException {
		...
		for (Item s = segs.getHead(); s != null; s = s.getNext()) {
			...
			if (LOGGER.isLoggable(Level.FINE)) {
				LOGGER.fine(" sr " + clunitDB.getSampleInfo().getSampleRate() + " " + s.getFeatures().getFloat("end") + " "
						+ (int) (s.getFeatures().getFloat("end") * clunitDB.getSampleInfo().getSampleRate()));
			}
			unitFeatureSet.setInt("target_end", (int) (s.getFeatures().getFloat("end") * clunitDB.getSampleInfo()
					.getSampleRate()));
	...
	/** Provides support for the Viterbi Algorithm. Implementation Notes
	 * <p>
	 * For each candidate for the current unit, calculate the cost between it and the first candidate in the next unit.
	 * Save only the path that has the least cost. By default, if two candidates come from units that are adjacent in the
	 * database, the cost is 0 (i.e., they were spoken together, so they are a perfect match).
	 * <p>
	 * Repeat the previous process for each candidate in the next unit, creating a list of least cost paths between the
	 * candidates between the current unit and the unit following it.
	 * <p>
	...
	static class Viterbi {
		...
		/** Creates a Viterbi class to process the given utterance. A queue of ViterbiPoints corresponding to the Items in
		 * the Relation segs is built up. */
		...
		/** Carry out a Viterbi search in for a prepared queue of ViterbiPoints. In a nutshell, each Point represents a
		 * target item (a target segment); for each target Point, a number of Candidate units in the voice database are
		 * determined; a Path structure is built up, based on local best transitions. Concretely, a Path consists of a
		 * (possibly empty) previous Path, a current Candidate, and a Score. This Score is a quality measure of the Path; it
		 * is calculated as the sum of the previous Path's score, the Candidate's score, and the Cost of joining the
		 * Candidate to the previous Path's Candidate. At each step, only one Path leading to each Candidate is retained,
		 * viz. the Path with the best Score. All that is left to do is to call result() to get the best-rated path from
		 * among the paths associated with the last Point, and to associate the resulting Candidates with the segment items
		 * they will realise. */
		void decode() {
			for (ViterbiPoint p = timeline; p.next != null; p = p.next) {
				...
				if (numStates != 0) {
					...
					for (int i = 0; i < p.numStates; i++) {
						if ((p == timeline && i == 0) || (p.statePaths[i] != null)) {
							// We are at the very beginning of the search, or have a usable path to extend
							// debug("   dc p " + p);
							for (ViterbiCandidate c = p.cands; c != null; c = c.next) {
		...
		/** Add the new path to the state path if it is better than the current path. In this, state means the position of
		 * the candidate associated with this path in the candidate queue for the corresponding segment item. In other
		 * words, this method uses newPath as the one path leading to the candidate newPath.candidate, if it has a better
		 ...
		 * @param b value to check. return true if a is better than b. */
		...
		/** Finds the best (queue of) candidate(s) for a given (segment) item. This traverses a CART tree for target cluster
		 * selection as described in the paper introducing the clunits algorithm. This corresponds to the "target costs"
		 * described for general unit selection.
		...
		private ViterbiCandidate getCandidate(Item item) {
			...
			if (clunitDB.getExtendSelections() > 0 && item.getPrevious() != null) {
				...
				for (int e = 0; lc != null && (e < clunitDB.getExtendSelections()); lc = lc.next) {
					...
					if (LOGGER.isLoggable(Level.FINE)) {
						LOGGER.fine("nu " + clunitDB.getUnit(nu).getName() + " all " + clunitDB.getUnit(all.ival).getName() + " "
								+ all.ival);
					}
		...
		/** Construct a new path element linking a previous path to the given candidate. The (penalty) score associated with
		 * the new path is calculated as the sum of the score of the old path plus the score of the candidate itself plus
		 * the join cost of appending the candidate to the nearest candidate in the given path. This join cost takes into
		...
		Cost getOptimalCouple(int u0, int u1) {
			...
			for (i = 0; i < fcount; ++i) {
				...
				dist = getFrameDistance(a, b, clunitDB.getJoinWeights(), clunitDB.getMcep().getSampleInfo()
						.getNumberOfChannels())
						+ Math.abs(clunitDB.getSts().getFrameSize(a) - clunitDB.getSts().getFrameSize(b))
		...
		/** Returns the distance between the successive potential frames.
		...
		int getOptimalCoupleFrame(int u0, int u1) {
			...
			return getFrameDistance(a, b, clunitDB.getJoinWeights(), clunitDB.getMcep().getSampleInfo().getNumberOfChannels())
					+ Math.abs(clunitDB.getSts().getFrameSize(a) - clunitDB.getSts().getFrameSize(b))
					* clunitDB.getContinuityWeight();
		}
	...
	/** Represents a point in the Viterbi path. A point corresponds to an item, e.g. a Segment. Each ViterbiPoint knows
	...
	static class ViterbiPoint {
		...
		/** Initializes the dynamic path array. The path array will have as many ViterbiPath members as there are candidates
		 * in the queue starting with candidate. Side effect on parameter: This will set the pos member of the candidates in
		 * the queue starting with candidate to the position in the queue.
		 * @param candidate the first candidate of interest */
		public void initDynamicPathArray(ViterbiCandidate candidate) {
			int i = 0;
			for (ViterbiCandidate cc = candidate; cc != null; i++, cc = cc.next) {
				cc.pos = i;
			}
			if (LOGGER.isLoggable(Level.FINE)) {
	...
	/** Represents a candidate for the Viterbi algorthm. Each candidate knows about its next candidate, i.e. they can form
	 * a queue. */
	static class ViterbiCandidate {
		...
		/** Sets the integer value for this candidate. This can be used for saving the unit index of the candidate unit
		 * represented by this ViterbiCandidate.
	...
	static class ViterbiPath {
		 ...
		 * @return <code>true</code> if the feature is present; otherwise <code>false</code>. */
