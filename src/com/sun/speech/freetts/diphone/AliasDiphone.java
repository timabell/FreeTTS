...
/** Represent an alias diphone which is just another name for an already-existing original diphone. This can be used just
 * like a "real" diphone.
...
public class AliasDiphone extends Diphone {
	 ...
	 * @param original a diphone object whose getName() must return the same as our getOriginalName().
	 * @throws IllegalArgumentException if the diphone to be registered as the original has a name which is different from
	 * this AliasDiphone's original name as returned by getOriginalName(). */
	...
	/** Returns the midpoint index. the midpoint index is the sample that divides the diphone into the first and second
	 * parts.
	 ...
	 * @param unitPart do we want the first have (1) or the second half (2)
	 * @return the sample nearest to the given index in the given part */
	...
	/** Determines if the two diphones are equivalent. This is for testing databases. This is not the same as "equals". For
	 * aliased diphones, this will return true if the compare() method of the original diphone returns true, even if the
	 * name used in this alias is different.
	 * @param other the diphone to compare this one to
	 * @return <code>true</code> if the diphones match; otherwise <code>false</code> */
