...
public class DiphoneUnitSelector implements UtteranceProcessor {
	...
	/** Creates the Unit Relation in the given utterance from the diphone units and their some associated information from
	 * the units database.
	...
	private void createUnitRelation(Utterance utterance) {
		...
		for (segmentItem0 = segmentRelation.getHead(); segmentItem0 != null && segmentItem0.getNext() != null; segmentItem0 = segmentItem1) {
			segmentItem1 = segmentItem0.getNext();
			diphoneName = segmentItem0.getFeatures().getString("name") + "-" + segmentItem1.getFeatures().getString("name");
	...
	/** Returns a new Item (a Unit) in the given Relation, and sets the new Item to the given diphone name, target end,
...
/** A wrapper around the Diphone class that turns the diphone into a unit */
