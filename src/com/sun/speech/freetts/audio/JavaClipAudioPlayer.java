...
/** Provides an implementation of <code>AudioPlayer</code> that creates javax.sound.sampled audio clips and outputs them
 * via the javax.sound API. The interface provides a highly reliable audio output package. Since audio is batched and
 * not sent to the audio layer until an entire utterance has been processed, this player has higher latency (50 msecs
 * for a typical 4 second utterance). */
public class JavaClipAudioPlayer implements AudioPlayer {
	...
	public JavaClipAudioPlayer() {
		...
		totalOpenFailDelayMs = Utilities.getLong("com.sun.speech.freetts.audio.AudioPlayer.totalOpenFailDelayMs", 0)
				.longValue();
	...
	/** Pauses audio output. All audio output is stopped. Output can be resumed at the current point by calling
	 * <code>resume</code>. Output can be aborted by calling <code> cancel </code> */
	public void pause() {
		if (!paused) {
			setPaused(true);
			if (currentClip != null) {
	 ...
	 * @return <code>true</code> if the write completed successfully, <code> false </code>if the write was cancelled. */
	public boolean drain() {
		timer.stop("speakableOut");
		return true;
	}

	/** Closes this audio player [[[ WORKAROUND TODO The javax.sound.sampled drain is almost working properly. On linux,
	 * there is still a little bit of sound that needs to go out, even after drain is called. Thus, the drainDelay. We
	 * wait for a few hundred milliseconds while the data is really drained out of the system ]]] */
	...
	/** Returns the current position in the output stream since the last <code>resetTime</code> Currently not supported.
	...
	public synchronized boolean end() {
		...
		if ((currentClip == null) || !currentClip.isOpen()) {
			...
			try {
				...
				while (currentClip != null && (currentClip.isRunning() || paused) && !cancelled) {
					wait();
				}
			} catch (InterruptedException ie) {
				ok = false;
