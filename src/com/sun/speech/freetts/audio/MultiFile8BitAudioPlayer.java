...
/** Streams audio to multiple files as 8-bit samples, one per utterance. Currently, FreeTTS always outputs 16-bit
 * samples, and this MultiFile8BitAudioPlayer will convert them to 8-bit before outputting them. */
public class MultiFile8BitAudioPlayer implements AudioPlayer {
	// 8-bit unsigned little-endian mono audio
	private AudioFormat currentFormat = new AudioFormat(8000, 8, 1, false, false);
	...
	/** Creates a default audio player for an AudioFileFormat of type WAVE. Reads the
	 * "com.sun.speech.freetts.AudioPlayer.baseName" property for the base filename to use, and will produce files of the
	 * form &lt;baseName>1.wav. The default value for the base name is "freetts". */
	...
	public boolean end() throws IOException {
		...
		AudioInputStream ais = new AudioInputStream(bais, currentFormat, outputData.length / currentFormat.getFrameSize());
	...
	/** Converts an array of signed 16-bit audio data to unsigned 8-bit audio data.
