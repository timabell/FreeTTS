...
/** Supports generating audio output from an utterance. This is an utterance processor. The primary method,
 * <code> procesUtterance </code> takes an utterance and hands it off to the LPCResult to be sent to the proper audio
 * player.
 * @see LPCResult */
public class AudioOutput implements UtteranceProcessor {
	...
	private final static AudioFormat AUDIO_8KHZ = new AudioFormat(8000.0f, 16, 1, true, true);
	private final static AudioFormat AUDIO_16KHZ = new AudioFormat(16000.0f, 16, 1, true, true);
	...
	/** Gets the current audio format. Given a sample info return an appropriate audio format. A cache of common audio
	 * formats is used to reduce unnecessary object creation. Note that this method always returns an AudioFormat that
	 * uses 16-bit samples.
