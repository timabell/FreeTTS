...
public class LPCResult {
	...
	/** this is a normalized version of the residuals; to normalize it, add 128 to it */
	...
	private final static int MAX_SAMPLE_SIZE = Utilities.getInteger("com.sun.speech.freetts.LpcResult.maxSamples", 1024)
			.intValue();
	...
	/** Returns the time difference of the frame at the given position with the frame prior to that. If the frame at the
	 * given position is the first frame (position 0), the time of that frame is returned.
	 * @param frameIndex the position of the frame
	 * @return the time difference of the frame at the given position with the frame prior to that */
	...
	/** Copies the information in the given unit to the array of residuals, starting at the given index, up until
	 * targetSize chars.
	...
	/** Copies the residual puse in the given unit to the array of residuals, starting at the given index, up until
	 * targetSize chars.
	 * @param source the unit that holds the information source
	 * @param targetPosition start position in the array of residuals
	 * @param targetSize the maximum number of characters to copy */
	...
	/** Given a 16 bit value (represented as an int), extract the high eight bits and return them
	...
	/** Given a 16 bit value (represented as an int), extract the low eight bits and return them
	 * @param val the 16 bit value
	 ...
	 * @param utterance the utterance [[[ TODO: well there is a bunch of duplicated code here .. these should be combined
	 * into one routine. ]]] */
	...
	private Wave getWave() {
		...
		AudioFormat audioFormat = new AudioFormat(getSampleRate(), Wave.DEFAULT_SAMPLE_SIZE_IN_BITS, 1,
	...
	/** A Wave is an immutable class that contains the AudioFormat and the actual wave samples, which currently is in the
	 * form of AudioInputStream. */
	private static class Wave {
		/** The default sample size of the Wave, which is 16. */
		public static final int DEFAULT_SAMPLE_SIZE_IN_BITS = 16;

		/** A boolean indicating that the Wave is signed, i.e., this value is true. */
		public static final boolean DEFAULT_SIGNED = true;

		/** A boolean indicating that the Wave samples are represented as little endian, i.e., this value is false. */
...
/** FloatList is used to maintain a circular buffer of float values. It is essentially an index-free array of floats that
 * can easily be iterated through forwards or backwards. Keeping values in an index free list like this eliminates index
 * bounds checking which can save us some time. */
class FloatList {
	float value;
	FloatList next;
	FloatList prev;
