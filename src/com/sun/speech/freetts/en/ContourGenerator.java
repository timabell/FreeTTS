...
/** Calculates the F0 curve for an utterance based on the Black and Hunt article "Generating F0 Contours from ToBI Labels
 * Using Linear Regression," ICSLP96, vol.&nbsp;3, pp 1385-1388, Philadelphia, PA.&nbsp;1996. */
public class ContourGenerator implements UtteranceProcessor {
	private final static PathExtractor endPath = new PathExtractorImpl("R:SylStructure.daughter.R:Segment.p.end", true);
	private final static PathExtractor lastDaughterEndPath = new PathExtractorImpl("R:SylStructure.daughtern.end", true);
	private final static PathExtractor postBreakPath = new PathExtractorImpl("R:SylStructure.daughter.R:Segment.p.name",
			true);
	private final static PathExtractor preBreakPath = new PathExtractorImpl("R:SylStructure.daughtern.R:Segment.n.name",
			true);
	private final static PathExtractor vowelMidPath = new PathExtractorImpl("R:Segment.p.end", true);
	 ...
	 * @throws ProcessException if an <code>IOException</code> is thrown during the processing of the utterance */
	public void processUtterance(Utterance utterance) throws ProcessException {
		...
		for (Item syllable = utterance.getRelation(Relation.SYLLABLE).getHead(); syllable != null; syllable = syllable
				.getNext()) {
			...
			if (syllable.getItemAs(Relation.SYLLABLE_STRUCTURE).hasDaughters()) {
				...
				addTargetPoint(target, val.floatValue(), mapF0((interceptor.start + lend) / 2.0f, localMean, localStddev));
				...
				if (isPreBreak(syllable)) {
					...
					addTargetPoint(target, eval.floatValue(), mapF0(interceptor.end, localMean, localStddev));
				}
			}
		}
	...
	private final float vowelMid(Item syllable) {
		...
		for (segment = firstSeg; segment != null; segment = segment.getNext()) {
			...
			if ("+".equals(voice.getPhoneFeature(segment.toString(), "vc"))) {
				val = (segment.getFeatures().getFloat("end") + ((Float) vowelMidPath.findFeature(segment)).floatValue()) / 2.0f;
				return val;
			}
		}
		...
		if (firstSeg == null) {
			...
			val = (firstSeg.getFeatures().getFloat("end") + ((Float) vowelMidPath.findFeature(firstSeg)).floatValue()) / 2.0f;
		}
	 ...
	 * @return <code>true</code> if this syllable is following a break; otherwise <code>false</code>. */
	 ...
	 * @param syllable the syllable to check
	 * @return <code>true</code> if this syllable is before a break; otherwise <code>false</code>. */
...
class Interceptor {
	 ...
	 * @return the string representation of the object */
	public String toString() {
		return Float.toString(start) + " " + Float.toString(mid) + " " + Float.toString(end);
	}
}
