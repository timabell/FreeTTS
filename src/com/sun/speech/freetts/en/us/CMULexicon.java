...
public class CMULexicon extends LexiconImpl {
	...
	private static final Logger LOGGER = Logger.getLogger(CMULexicon.class.getName());
	 ...
	 * @param letterToSoundURL the letter to sound rules are loaded from here
	 * @param binary if <code>true</code> the input data are loaded as binary ; otherwise if <code>false</code> the input
	 ...
	 * @return <code>true</code> if the word phone in question is on a syllable boundary; otherwise <code>false</code>. */
	 ...
	 * @return <code>true</code> if the phone represents a silent phone; otherwise <code>false</code>. */
	...
	/** Provides test code for the CMULexicon. <br>
	 * <b>Usage:</b><br>
	 * 
	 * <pre>
			...
			if (args.length > 0) {
				...
				for (int i = 0; i < args.length; i++) {
					if (args[i].equals("-src")) {
						...
						lex = new CMULexicon(new URL(path + "_compiled.txt"), new URL(path + "_addenda.txt"), new URL(path
								+ "_lts.txt"), false);
