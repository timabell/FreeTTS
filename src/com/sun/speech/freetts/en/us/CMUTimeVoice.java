...
/** Defines limited domain synthesis voice that specializes in telling the time. This is based on data created by the
 * example time domain voice in FestVox. */
public class CMUTimeVoice extends CMUClusterUnitVoice {
	...
	public CMUTimeVoice(String name, Gender gender, Age age, String description, Locale locale, String domain,
	...
	/** The FestVox voice does not take advantage of any post lexical processing. As a result, it doesn't end up getting
	 * certain units that are expected by the typical post lexical processing. For example, if "the" is followed by a word
	 * that begins with a vowel, the typical post lexical processing will change its pronunciation from "dh ax" to
	 * "dh iy". We don't want this in this voice.
