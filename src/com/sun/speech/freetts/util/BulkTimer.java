...
public class BulkTimer {
	...
	/** Starts the timer with the given name. A BulkTimer can manage any number of timers. The timers are referenced by
	 * name. A timer is created the first time it is referenced.
	...
	/** Starts the bulk timer. The BulkTimer maintains a timer for itself (called SELF). This is used to measure the
	 * overall time for a bulk timer. When timing data is displayed, the percentage of total time is displayed. The total
	 * time is the time between <code> start </code> and <code> end </code> calls on the <code> BulkTimer </code> . */
	public void start() {
		getTimer(SELF).start();
	}
	 ...
	 * @return <code>true</code> if verbose mode; otherwise <code>false</code>. */
