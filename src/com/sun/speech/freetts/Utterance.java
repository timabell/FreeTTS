...
/** Holds all the data for an utterance to be spoken. It is incrementally modified by various UtteranceProcessor
 * implementations. An utterance contains a set of Features (essential a set of properties) and a set of Relations. A
 * Relation is an ordered set of Item graphs. The utterance contains a set of features and implements FeatureSet so that
 * applications can set/get features directly from the utterance. If a feature query is not found in the utterance
 * feature set, the query is forwarded to the FeatureSet of the voice associated with the utterance. */
...
public class Utterance implements FeatureSet, Serializable {
	...
	/** Determines if the given feature is present. If the feature is not present in the utterance, the feature set of the
	 * voice is checked.
	...
	/** Convenience method that returns the named feature as a string. If the named feature is not present in the
	 * utterance, then this attempts to retrieve it from the voice.
	...
	/** Convenience method that returns the named feature as a int. If the named feature is not present in the utterance,
	...
	/** Convenience method that returns the named feature as a float. If the named feature is not present in the utterance,
	 * then this attempts to retrieve it from the voice.
	 * @param name the name of the feature
	 * @return the value associated with the name or null if the value is not found
	...
	/** Returns the named feature as an object. If the named feature is not present in the utterance, then this attempts to
	 * retrieve it from the voice.
	 * @param name the name of the feature
	 * @return the value associated with the name or null if the value is not found */
	 ...
	 * @param time the time
	 * @throws IllegalStateException if the Segment durations have not been calculated in the Utterance or if the given
	 * relation is not present in the Utterance */
	...
	/** Returns the duration of this Utterance in seconds. It does this by looking at last Item in the following Relations,
	 * in this order: Segment, Target. If none of these Relations exist, or if these Relations contain no Items, an
	...
	/** Returns the float feature of the last Item in the named Relation.
	 * @return the float feature of the last Item in the named Relation, or -1 otherwise */
	...
	/** Sets the token list for this utterance. Note that this could be optimized by turning the token list directly into
	 * the token relation.
	 ...
	 * @return true if this is the first utterance in a series of connected utterances. */
	 ...
	 * @return true if this is the last utterance in a series of connected utterances. */
